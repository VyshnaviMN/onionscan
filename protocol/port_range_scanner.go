...
func getStatus(onion string, port int, proxyAddress string, timeout time.Duration) (string, time.Duration) {
...
func (sps *OtherPortsScanner) ScanProtocol(onion string, onionId string, osc *config.OnionScanConfig, report *report.OnionScanReport, rescanQueue *OnionQueue) {
	...
	// var openPortsBuilder bytes.Buffer
	...
	status, timeDiff = getStatus(hiddenService, checkPort, osc.TorProxyAddress, osc.Timeout)
